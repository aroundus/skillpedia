...
| 환경 변수                   | 설명                                                                                                                                                                             |
| --------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `FEEDLOOP_ID`               | Feedloop 챗봇 위젯으로 피드백을 받는 경우 [Feedloop](https://ai-feedloop.vercel.app/bots/new)에 서비스를 직접 등록하고 발급받은 서비스 ID를 지정하며, production 모드에서만 적용 |
| `GITHUB_TOKEN`              | 앱을 설치하지 않은 공개 저장소를 목록에 함께 노출하는 경우 사용                                                                                                                  |
| `GTM_ID`                    | Google 태그 관리자로 데이터를 수집하는 경우 컨테이너 ID를 지정하며, production 모드에서만 적용 ([데이터 수집](./EVENT_TRACKER.ko.md))                                            |
...
| 종류   | 이름                                                        |
| ------ | ----------------------------------------------------------- |
| 변수   | `APP_ID`, `FEEDLOOP_ID`, `GTM_ID`, `REPOSITORIES`           |
